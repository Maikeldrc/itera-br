...
export function ClaimDetailPanel({
  ...
  const financialTimeline = [
    ...serviceLines.flatMap((line, index) => {
      ...
      const events = [
        line.paymentDate ? {
          ...
          cpt,
          type: "Payment",
          detail: `Paid ${formatFinancialValue(Number(line.paid || 0) + Number(line.secondaryPaid || 0))}; balance ${formatFinancialValue(Number(line.balance || 0))}`
        } : null,
        Number(line.adj || 0) > 0 ? {
          at: line.paymentDate || claim.updated_at || claim.created_at || "",
          cpt,
          type: "Adjustment",
    ...
    ...filteredAudits
      .filter(log => /paid|payment|adjust|balance|payer|status|service/i.test(`${log.field_name} ${log.action_type}`))
      .map(log => ({
        at: log.changed_at,
        cpt: "-",
        type: log.action_type || "Audit",
        detail: `${log.field_name}: ${log.previous_value || "-"} -> ${log.new_value || "-"}`
      }))
  ...
  const renderFinancialTimeline = (className = "") => (
    <div className={`rounded-xl border border-slate-200 bg-white shadow-sm ${className}`}>
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
        <div className="flex items-center gap-2">
          <History className="h-4 w-4 text-primary-blue" />
          <h4 className="text-xs font-bold text-slate-800">{isEnglish ? "CPT Financial Timeline" : "Historial financiero por CPT"}</h4>
        </div>
        <span className="rounded-md bg-slate-100 px-2 py-1 text-[9px] font-bold text-slate-500">{financialTimeline.length}</span>
      </div>
      <div className="max-h-56 overflow-auto">
        {financialTimeline.length > 0 ? (
          <table className="w-full min-w-[760px] text-left text-[10px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {financialTimeline.map((event, index) => (
                <tr key={`${event.at}-${event.cpt}-${event.type}-${index}`}>
                  <td className="px-3 py-2 font-mono text-slate-500">{event.at ? new Date(event.at).toLocaleString() : "-"}</td>
                  <td className="px-3 py-2 font-mono font-bold text-dark-blue">{event.cpt}</td>
                  <td className="px-3 py-2">
                    <span className="rounded-md bg-blue-50 px-2 py-1 font-bold text-dark-blue">{event.type}</span>
                  </td>
          ...
          <div className="px-5 py-8 text-center text-xs font-semibold text-slate-400">
            {isEnglish ? "No financial history has been recorded for this claim yet." : "Aún no hay historial financiero registrado para este claim."}
          </div>
        )}
      </div>
    </div>
