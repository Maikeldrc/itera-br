  ...
  const tests: { name: string; run: () => void }[] = [
    ...
    },
    {
      ...
      run: () => {
        ...
        assert(canUserAccessMenu({ role: UserRole.ReconciliationSpecialist }, "payment-reconciliation-import"), "Reconciliation Specialist should access Payment Import.");
        assert(!canUserAccessMenu({ role: UserRole.ProviderViewer }, "payment-reconciliation-import"), "Provider Viewer must not access Payment Import.");
